...
(() => {
  ...
  // Remove existing input for this item if it exists
  const existingInput = document.getElementById(item["slug"]);
  if (existingInput) {
    form.removeChild(existingInput);
  }

  const hiddenInput = document.createElement("input");
  hiddenInput.name = `Item: ${item["name"]}`;
  hiddenInput.id = item["slug"];
  hiddenInput.type = "hidden";
  hiddenInput.value = `Category: ${item["category"] || "N/A"}, Quantity: ${item["quantity"]}`;
