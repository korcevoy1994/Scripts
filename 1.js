...
(() => {
 ...
 // Добавляем элемент в форму
  ...
  // Создаем скрытый input для передачи значения с тегом "Putup"
  const putupInput = document.createElement("input");
  putupInput.name = "Putup";
  putupInput.type = "hidden";
  putupInput.value = item["putup"] || "Default Putup"; // Берем значение из CMS коллекции
  putupInput.className = "quote-item";

  // Добавляем элемент с тегом "Putup" в форму
  document.getElementById("wf-form-Request-Quote")?.appendChild(putupInput);

  // Создаем скрытый input для передачи категории
  const categoryInput = document.createElement("input");
  categoryInput.name = "Category";
  categoryInput.type = "hidden";
  categoryInput.value = item["category"] || "Default Category"; // Берем значение из CMS коллекции
  categoryInput.className = "quote-item";

  // Добавляем элемент с категорией в форму
  document.getElementById("wf-form-Request-Quote")?.appendChild(categoryInput);
}
